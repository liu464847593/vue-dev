...
export function set (target: Array<any> | Object, key: any, val: any): any {
  ...
  if (Array.isArray(target) && isValidArrayIndex(key)) { // 如果target是一个数组并且key是一个有效的索引值
    target.length = Math.max(target.length, key)
    target.splice(key, 1, val)
    return val
  }
  if (key in target && !(key in Object.prototype)) { // key 已经存在target中的情况
    target[key] = val
    return val
  }
  // 新增
  const ob = (target: any).__ob__
  if (target._isVue || (ob && ob.vmCount)) { // 不能是vue实例，或vue.js实例的根数据对象
